...
output "argocd_gcp_iam_service_account_email" {
  value = local.enable_workload_identity ? module.my-app-workload-identity[0].gcp_service_account_email : var.service_acc_name
}

output "argocd_gcp_iam_service_account_name" {
  value = local.enable_workload_identity ? module.my-app-workload-identity[0].gcp_service_account_name : var.service_acc_name
}
